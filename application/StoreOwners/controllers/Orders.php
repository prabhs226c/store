...
class Orders extends MY_Controller {
	...
	public function index() {
        ...
        $group_by = 'o.id';
        $this->dataModule['results']=  $this->Sitefunction->get_all_rows(TBL_STORE_ORDERS.' as o', $columns, $where, $join, array('o.id'=>'desc'), '', 'LEFT', array(), $group_by, array(), array());
    ...
    public function view($id) {
        ...
        $this->dataModule['order_info']=  $this->Sitefunction->get_single_by_query("SELECT o.*, r.name, r.profile_image, r.address, a.address_line_1, a.address_line_2, a.phone, a.name as user_name, a.city, u.phone as customer_phone, u.email as customer_email, u.fullname as customer_name FROM ".TBL_STORE_ORDERS." as o LEFT JOIN ".TBL_STORES." as r ON r.id=o.store_id LEFT JOIN ".TBL_ADDRESS." as a ON a.id=o.address_id  LEFT JOIN ".TBL_USERS." as u ON u.id=o.user_id WHERE o.id=".$id." and o.status=1 and r.owner_id=".$this->user_id);
        $this->dataModule['controller']=$this; 

        $where = array('od.order_id='=>$id, 'od.status'=>1);
        $columns = "od.*, p.type, p.title, p.image";
        $join = array(TBL_STORE_SUBCATEGORIES.' as p'=>"od.product_id=p.id");
        $group_by = 'od.id';
        $this->dataModule['get_order_details']=  $this->Sitefunction->get_all_rows(TBL_STORE_ORDERDETAIL.' as od', $columns, $where, $join, array(), '', 'LEFT', array(), $group_by, array(), array());
    ...
    public function change_order_status() {
        ...
        if($this->Sitefunction->update(TBL_STORE_ORDERS, array("order_status"=>$this->input->get_post('order_status')), array('id'=>$this->input->get_post('orderid')))){
