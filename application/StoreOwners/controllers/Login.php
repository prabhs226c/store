	...
	public function index() {
		...
		if(!empty($post)) {
			...
			if ($this->form_validation->run() != FALSE)
			{
				...
				$admin_find = $this->Sitefunction->get_single_row(TBL_STORE_OWNERS, '*', array('email'=>urlencode($username), 'password'=>md5($password)));

				if(!empty($admin_find)) {

					$data= array('store_owner_id'=>$admin_find->id);
