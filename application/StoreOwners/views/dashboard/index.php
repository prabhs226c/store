...
for($m=1; $m<=12; $m++){
    $m= $m<=9?('0'.$m):$m;
    $date=  $m."-".date('Y');
    $montly_profit= $this->Sitefunction->get_all_rows(TBL_EARNINGS. ' as e ', "SUM(e.owners_amount) AS earnings", array('e.status'=>1, "r.owner_id"=>$this->user_id, "DATE_FORMAT(e.created, '%m-%Y')="=>"$date"), array(TBL_STORES.' as r'=>"e.store_id=r.id"));
