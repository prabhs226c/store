...
class Webservices extends MY_Controller
{
	...
	public function home()
	{
		...
		* cos( radians( longitude ) - radians(" . $long . ") ) + sin( radians(" . $lat . ") ) * sin(radians(latitude)) ) ) AS distance,  ROUND( AVG(rr.review),1 ) as review FROM " . TBL_RESTAURANTS . " as r INNER JOIN " . TBL_STATE . " as s ON s.id=r.state_id INNER JOIN " . TBL_CITY . " as ct ON ct.id=r.city_id INNER JOIN " . TBL_COUNTRY . " as c ON c.id=r.country_id LEFT JOIN " . TBL_RESTAURANT_REVIEW . " as rr ON rr.restaurant_id=r.id GROUP BY r.id HAVING r_status=1  and s_status=1 and ct_status=1 and c_status=1 ORDER BY distance ASC LIMIT 0,5");
		...
		/*
		//getting Banner of the coupon code
		$get_banner_restaurants = $this->Common_model->get_all_rows_by_query("SELECT id, coupon_code, description, discount_type, discount, image FROM " . TBL_COUPONS);

		if (!empty($get_banner_restaurants)) {
			foreach ($get_banner_restaurants as $row) {
				$bannerRes['id'] = $row['id'];
				$bannerRes['coupon_code'] = $row['coupon_code'];
				$bannerRes['description'] = $row['description'];
				$bannerRes['image'] = $this->getImage(explode(',', $row['image'])[0], 'coupons');
				$bannerRes['discount'] = $row['discount'];
				$bannerRes['discount_type'] = $row['discount_type'];
		...
		* cos( radians( longitude ) - radians(" . $long . ") ) + sin( radians(" . $lat . ") ) * sin(radians(latitude)) ) ) AS distance FROM " . TBL_SUBCATEGORIES . " as f INNER JOIN " . TBL_RESTAURANTS . " as r  ON r.id=f.restaurant_id INNER JOIN " . TBL_STATE . " as s ON s.id=r.state_id INNER JOIN " . TBL_CITY . " as ct ON ct.id=r.city_id INNER JOIN " . TBL_COUNTRY . " as c ON c.id=r.country_id GROUP BY f.id HAVING r_status=1  and s_status=1 and ct_status=1 and c_status=1 ORDER BY distance ASC LIMIT 0,5");
		...
		//GET CUISINE
		$get_cuisine =  $this->Common_model->get_all_rows(TBL_CUISINE . ' as c', 'c.*, COUNT(s.cuisine_id) as total', array('c.status' => 1), array(TBL_RESTAURANTS . ' as s' => 's.cuisine_id=c.id'), array(), '5,0', 'LEFT', array(), 'c.id', array(), array());
		if (!empty($get_cuisine)) {

			foreach ($get_cuisine as $cats) {
				$catRes['cuisine_name'] = urldecode($cats['title']);
				$catRes['image'] = $this->getImage($cats['image'], 'cuisine');;
				$catRes['cuisine_id'] = $cats['id'];
				$catRes['total_count'] = $cats['total'];
				$catResponse1[] = $catRes;
			}

			$cuisineResponse = $catResponse1;
		}


		$get_categories =  $this->Common_model->get_all_rows(TBL_CATEGORIES . ' as c', 'c.*, COUNT(s.category_id) as total', array('c.status' => 1), array(TBL_SUBCATEGORIES . ' as s' => 's.category_id=c.id'), array(), '5,0', 'LEFT', array(), 'c.id', array(), array());
		...
		$get_codes =  $this->Common_model->get_all_rows(TBL_COUPONS . ' as c', 'c.*, ROUND(c.discount, 0) as discount', array('c.status' => 1, 'c.end_date >= ' => $current_date), array(), array('c.end_date' => 'asc'), '5,0', 'LEFT', array(), 'c.id', array(), array());
		if (!empty($get_codes)) {
	...
	public function details()
	{
		...
		if (!empty($get_restaurant)) {
			...
			//$restData['cuisine_id'] = $get_restaurant->cuisine_id;

			$where = array('r.id=' => $restaurant_id, 's.status' => 1, 'c.status' => 1);
				...
				foreach ($get_categories as $cats) {
					...
					if (!empty($get_subcat)) {
						foreach ($get_subcat as $subcat) {
							...
							$where = array('s.subcategory_id=' => $subcat['id'], 's.status!=' => 9, 's.restaurant_id' => $restaurant_id);
							$columns = "s.*";
							$join = array(TBL_SUBCATEGORIES . ' as c' => "c.id=s.subcategory_id",);
							$group_by = 's.id';
							$get_sides =  $this->Common_model->get_all_rows(TBL_SIDES . ' as s', $columns, $where, $join, array('s.title' => 'asc'), '', 'LEFT', array(), $group_by, array(), array());
							$sidesResponse = array();
							if (!empty($get_sides)) {
								foreach ($get_sides as $side) {
									$sidesRes['id'] = $side['id'];
									$sidesRes['is_available'] = $side['status'];
									$sidesRes['name'] = urldecode($side['title']);
									$sidesRes['image'] = $this->getImage($side['image'], 'sides');
									$sidesRes['price'] = $side['price'];
									$sidesRes['description'] = urldecode($side['description']);

									$sidesResponse[] = $sidesRes;
								}
							}

							$subcatRes['sides'] = $sidesResponse;
							...
							$join = array(TBL_SUBCATEGORIES . ' as c' => "c.id=s.subcategory_id",);
							$group_by = 's.id';
							$get_addon =  $this->Common_model->get_all_rows(TBL_ADDON . ' as s', $columns, $where, $join, array('s.title' => 'asc'), '', 'LEFT', array(), $group_by, array(), array());
							$addonResponse = array();
							if (!empty($get_addon)) {
								foreach ($get_addon as $addon) {
									$addonRes['id'] = $addon['id'];
									$addonRes['is_available'] = $addon['status'];
									$addonRes['name'] = urldecode($addon['title']);
									$addonRes['image'] = $this->getImage($addon['image'], 'sides');
									$addonRes['price'] = $addon['price'];
									$addonRes['description'] = urldecode($addon['description']);

									$addonResponse[] = $addonRes;
								}
							}

							$subcatRes['addon'] = $addonResponse;
							//```````````````````````````````ADDON`````````````````````````````````````
			...
			/*
			$where = array('r.id=' => $restaurant_id, 's.status' => 1, 'c.status' => 1);
			$columns = "c.*, s.id as sides_id, s.restaurant_id";
			$join = array(TBL_SIDES . ' as s' => "s.subcategory_id=c.id", TBL_RESTAURANTS . ' as r' => "r.id=s.restaurant_id");
			$group_by = 'c.id';
			$get_sides =  $this->Common_model->get_all_rows(TBL_SUBCATEGORIES . ' as c', $columns, $where, $join, array('c.title' => 'asc'), '', 'LEFT', array(), $group_by, array(), array());
			$sidesResponse = array();
			if (!empty($get_subcategories)) {
				foreach ($get_subcategories as $subcats) {
					$subcatRes['subcategory_name'] = urldecode($subcats['title']);
					$subcatRes['subcategory_id'] = $subcats['id'];
					$where = array('s.subcategory_id=' => $subcats['id'], 's.status!=' => 9, 's.restaurant_id' => $restaurant_id);
					$columns = "s.*";
					$join = array(TBL_SUBCATEGORIES . ' as c' => "c.id=s.subcategory_id",);
					$group_by = 's.id';
					$get_sides =  $this->Common_model->get_all_rows(TBL_SIDES . ' as s', $columns, $where, $join, array('s.title' => 'asc'), '', 'LEFT', array(), $group_by, array(), array());
	...
	function add_order()
	{
		...
		if ((isset($charge) && !empty($charge)) || (isset($paypalResult) && !empty($paypalResult->success))  || $requestData->payment_type == 1 || $requestData->total_price == '0.00' || $requestData->payment_type == 4 || $requestData->payment_type == 5) {
			...
			$addArr['created'] = date('Y-m-d H:i:s');
			$addArr['updated'] = date('Y-m-d H:i:s');
			$addArr['delivery_charges'] = $requestData->deliveryCharges;
			$addArr['tax_price'] = $requestData->taxAmount;
			$addArr['admin_charge'] = $requestData->charge;
			$addArr['is_contactless'] = $requestData->isContactLess;
			...
			if ($insert) {
				...
				foreach ($order_details as $oData) {
					...
					$orderDetails = $this->Common_model->insert(TBL_ORDERDETAIL, $addArrO);
					if ($orderDetails) {
						$orderDetails_id = $this->db->order_details();
						$order_items_details = $requestData->addSide;
						foreach ($order_items_details as $oiData) {
							$addArr1['product_id'] = $oiData->product_id;
							$addArr1['order_details_id'] = $oiData->$orderDetails_id;
							$addArr1['is_addon'] = $oiData->product_id;
						}
					}
				}
