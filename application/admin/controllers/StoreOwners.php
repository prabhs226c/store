...
class StoreOwners extends MY_Controller {
	...
	public function index() {
        ...
        $group_by = 'o.id';
        $this->dataModule['results']=  $this->Sitefunction->get_all_rows(TBL_STORE_OWNERS.' as o', $columns, $where, $join, array(), '', 'LEFT', array(), $group_by, array(), array());

		$this->load->view('store_owners/index', $this->dataModule);
	}

	public function delete() {

        $this->Sitefunction->delete(TBL_STORE_OWNERS, array('id'=>$this->input->get_post('id')));
    ...
    public function multiple_delete() {
        ...
        foreach($ids as $id){
            $this->Sitefunction->delete(TBL_STORE_OWNERS, array('id'=>$id));
        }
    ...
    public function add(){
        ...
        if($postData && !empty($postData)) {
			...
			if ($this->form_validation->run() != FALSE)
			{
               ...
               if($this->Sitefunction->insert(TBL_STORE_OWNERS, $data_array)){
                    $this->session->set_flashdata('success', $this->lang->line('owner_added_successfully'));
                    redirect(STORE_OWNER_PATH);
                }else {
        ...
        $this->load->view('store_owners/add',$this->dataModule);
    }

    public function edit($id){
        $this->dataModule['results']= $this->Sitefunction->get_single_row(TBL_STORE_OWNERS, '*', array('id'=>$id));
        if(!$id || empty($this->dataModule['results'])) {
        	$this->session->set_flashdata('here',"here ia ma");
            redirect(ERROR_PATH);
        }
        $postData= $this->input->post();
        if($postData && !empty($postData)) {
			...
			if ($this->form_validation->run() != FALSE)
			{
                ...
                $data_array['updated']=$this->utc_time;
                if($this->Sitefunction->update(TBL_STORE_OWNERS, $data_array, array('id'=>$id))){
        ...
        $this->load->view('store_owners/edit',$this->dataModule);
    }
