...
class Addon extends MY_Controller {
	...
	public function index() {
        ...
        $group_by = '';
        $this->dataModule['results']=  $this->Sitefunction->get_all_rows(TBL_ADDON.' as s', $columns, $where, $join, array(), '', 'LEFT', array(), $group_by, array(), array());
		$this->load->view('addon/index', $this->dataModule);
	}
	public function add($restarent_id=false){
			...
			if ($this->form_validation->run() != FALSE)
			{
                ...
                if(isset($_FILES['image']) && !empty($_FILES['image']))
                {
                    ...
                    if ($_FILES["image"]["error"] <=0) { 
                        move_uploaded_file($_FILES['image']['tmp_name'], UPLOAD_PATH.'addon/'. $file_name1);
                        $data_array['image']= $file_name1;
                    } 
                
                
                }
                if($this->Sitefunction->insert(TBL_ADDON, $data_array)){
                    $this->session->set_flashdata('success', $this->lang->line('addon_added_successfully'));
                    redirect(ADDON_PATH);
                }else {
        ...
        $this->load->view('addon/add',$this->dataModule);       
    }
	public function delete() {
		
        $this->Sitefunction->delete(TBL_ADDON, array('id'=>$this->input->get_post('id')));
    ...
    public function multiple_delete() {
        ...
        foreach($ids as $id){
            $this->Sitefunction->delete(TBL_ADDON, array('id'=>$id));
        }

    }

    public function edit($id) {
        $this->dataModule['results']= $this->Sitefunction->get_single_row(TBL_ADDON, '*', array('id'=>$id));
        ...
        if($postData && !empty($postData)) {
			...
			if ($this->form_validation->run() != FALSE)
			{
                ...
                if($this->Sitefunction->update(TBL_ADDON, $data_array, array('id'=>$id))){
                    $this->session->set_flashdata('success', $this->lang->line('addon_updated_successfully'));
                    redirect(ADDON_PATH);
                }else {
                    $this->session->set_flashdata('error', $this->lang->line('error_try_again'));
                }
        ...
        $this->load->view('addon/edit',$this->dataModule);  
    }
}
?>