...
class StoreSubCategories extends MY_Controller {
	...
	public function index() {
		...
		$this->load->view('store_subcategories/index', $this->dataModule);
	}
	public function add($restarent_id=false){
        ...
        if($postData && !empty($postData)) {
			...
			if ($this->form_validation->run() != FALSE)
			{
				...
				if($this->input->post('isNewCategory')==0) {
					...
					$this->db->insert('tbl_grocerycategories',['title'=>$this->input->post('newCategory'),'image'=>' ','status'=>1]);
                ...
                if($this->Sitefunction->insert(TBL_STORE_SUBCATEGORIES, $data_array)){
                    ...
                    redirect(STORE_SUBCATEGORY_PATH);
                }else {
        ...
        $this->load->view('store_subcategories/add',$this->dataModule);
    }
    ...
    public function edit($id) {
        ...
        $this->load->view('store_subcategories/edit',$this->dataModule);
    }
}
?>
